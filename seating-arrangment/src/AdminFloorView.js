...
function AdminFloorView({ locationSelected, floor, goBack }) {
  ...
  // pageSize handlePageSizeChange  setOpenDeletePopup setDeleteEmpData
  ...
  const handleClickOpenDelete = (row) => {
    ...
    setDeleteEmpData(row);
  };

  const handleClickCloseDeletePopup = () => {
    setOpenDeletePopup(false);
    setDeleteEmpData(undefined);
  };

  const handleClickOpenAddMore = () => {
    setAddMore(true);
  };

  const handleClickCloseAddMore = () => {
    setAddMore(false);
  };
  return (
    <>
      ...
      <Box padding={2}>
        <Grid container spacing={2}>
          ...
          >
            <Button
              onClick={handleClickOpenAddMore}
              variant="contained"
              color="primary"
            >
              Add More
      ...
      <Dialog
        ...
        onClose={handleClickCloseDeletePopup}
        aria-describedby="alert-dialog-slide-description"
      >
        <DialogTitle>{"Remove This Seat"}</DialogTitle>
        <DialogContent>
          <Typography variant="h6">
            Are You Sure You Want To Remove This Seat "{deleteEmpData?.seatId}"
          </Typography>
          <Box padding={1}>
            <Box display="flex" justifyContent="space-between">
              <Typography variant="h6">
                Day Shift Assigne : {deleteEmpData?.dayShift}
              </Typography>

              <Typography variant="h6">
                Night Shift Assigne : {deleteEmpData?.nightShift}
              </Typography>
            </Box>

            <Box display="flex" justifyContent="space-between">
              <Typography variant="h6">Hr Id : 41632</Typography>

              <Typography variant="h6">Hr Id : 41633</Typography>
            </Box>
          </Box>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClickCloseDeletePopup}>Discard</Button>
          <Button onClick={handleClickCloseDeletePopup}>Delete</Button>
        </DialogActions>
      </Dialog>

      {/* for add more sear    //handleClickOpenAddMore,addMore,handleClickCloseAddMore */}
